...
import { BudgetContext, BudgetContextType, BudgetProvider } from "./hooks/BudgetContext";
import { useContext, useEffect, useState } from "react";
import { fetchOneBudget } from "./services/fetchOneBudget";
import { ItemTableImpresiones, ItemTableManoObra, ItemTableMateriales, ItemTableServicioTerceros, ItemTableViaticos } from "./components/ModalsServicesKeyof";
...
export const BudgetDetail = () => {
  ...
  return (
    <BudgetProvider>
      <BudgetInformation id={id} />
    </BudgetProvider>
  );
};

export const BudgetInformation = ({ id }: { id?: string }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        const { budget, status } = await fetchOneBudget(id);
        if (budget && (status === 200 || status === 204)) {
          setBudget(budget);
